...
public class BatchProcessor implements PlugIn {
	...
	protected static String includeString = "";
	protected static String excludeString = "";
	...
	protected static String[] operations = {
		...
		"Vessel diameter analysis: Generate mask(2)", //9
		"Substract background only"	//10
	};
	...
	public static List<File> getTiffStackAsList (
		...
		System.out.println("debug batch tif file size: " + fileArray.length);
		for (int i=0; i<fileArray.length; i++) {
			boolean match = true;
			if (fileArray[i].isDirectory()) {
				if (isImageSequence(fileArray[i])) {
	...
	public static boolean isImageSequence (File input) {	
	...
	public static boolean addDialog() {
		...
		gd.addDirectoryOrFileField("root folder", rootDirPath, 70);
		...
		gd.addStringField("excluding:", excludeString, 30);
	...
	public void run(String arg0) {
		...
		*/
		...
		if (files.size()==0) {
			IJ.error("No data matches condition found!");
		...
		case 8: // Create projection images
			...
			break;
		case 9:	// Vessel diameter analysis: Generate mask to a different folder (without 3D selection and other operations)
			try {
				RSOM_Vessel_Analysis.batchRun2(fileArray);
			} catch (IOException e) {
				e.printStackTrace();
			}
			break;
		case 10: // Substract background only
			try {
				RSOM_Vessel_Analysis.batchRun3(fileArray);
			} catch (IOException e) {
				e.printStackTrace();
			}
			break;
	...
	public static void drawManaulRoi (
		...
		for (File f : fileArray) {
			...
			if (w.escPressed()) {
				...
				ori.close();
				rm.reset();
				IJ.log("  User cancelled manual ROI creation for file: ");
				IJ.log("    " + f.getCanonicalPath());
				continue;
			}
			if (mask != null)  mask.close();
	...
	public static void main(final String... args) throws Exception {
		...
		includeString = prefs.get(String.class, "persistedString", includeString);
		excludeString = prefs.get(String.class, "persistedString", excludeString);
		...
		prefs.put(String.class, "persistedString", includeString);
		prefs.put(String.class, "persistedString", excludeString);
