...
public class CreateProjectionImages implements PlugIn {
	...
	public static void batchRun(
		...
		filePath = fileArray[0].getCanonicalPath();
		...
		for (File f : fileArray) {	// file could be non-RSOM images
			...
			File projectionImageDir = new File(f.getParent() + File.separator + inputImage.getShortTitle() + "_projections");
			...
			for (int i=0; i<XYZ.length; i++) {
				...
				for (int j=0; j<proj.length; j++) {
					if (proj[j]!=null) {
						...
						IJ.saveAs(proj[j], "Tiff", projectionImageDir + File.separator + proj[j].getTitle() + ".tif");
					}
				}
			}
			...
			IJ.log("  File: " + f.getName() + " processed, took " + String.valueOf(duration) + " seconds.");
		}
		temp.close();
		System.gc();
	}
