...
public class ThresholdImage {
	...
	public boolean checkThreshold (ThresholdImage t) {
		...
		//double[] criterionWarning = {0.0001, 0.15}; // the volume fraction of blood vessel (create warning)
		//double[] criterion = {0.000001, 0.999999}; // 2019.06.20 modification, to allow larger range of the thresholding result
		if (t.mask==null)	{
			return true;
		}
		...
		double volFraction = 1-(ssm.pixelCount/ssm.maxCount);
		//double volFraction = ssm.maxCount/ssm.pixelCount;
		
		if (volFraction < criterionWarning[0]) {
			if (volFraction < criterion[0]) {
				IJ.log("Error: thresholded voxel less than "+String.valueOf(criterion[0]*100)+"%");
				IJ.log("Try to decrease the lower bound of the thresholding range");
				return true;
			}
			String thresholdedVxTooLittle = "Thresholded voxel less than "+String.valueOf(criterionWarning[0]*100)+"%"
					+ "\nThis could generate wrong result in the end. Click OK to continue.";
			IJ.error(thresholdedVxTooLittle);
		}
		
		if (volFraction > criterionWarning[1]) {
			if (volFraction > criterion[1]) {
				IJ.log("Error: thresholded voxel more than "+String.valueOf(criterion[1]*100)+"%");
				IJ.log("Try to increase the lower bound of the thresholding range");
				return true;
			}
			String thresholdedVxTooMany = "Thresholded voxel more than "+String.valueOf(criterionWarning[1]*100)+"%"
					+ "\nIt might take much longer time to finish the processing."
					+ "\nThis could also generate wrong result in the end. Click OK to continue.";
			IJ.error(thresholdedVxTooMany);
		}
		*/
		return false;
	}
